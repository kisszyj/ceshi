<template>
    <div class="pres posit-box">
        <head-top v-if="!$alipayBool && $isMobile" @backClick="changeBack" v-bind:data="{title:$t('message.head.pre'),start:true}"></head-top>
        <booking-head-web v-if="!$isMobile" @backClick="changeBack" v-bind:data="{title:$t('message.head.pre')}" ></booking-head-web>
        <div class="pres-tab" :class="{'addTopHeight, addRel': !($alipayBool)}">
            <div class="pres-list" v-if="$isMobile">
            ...
            <prince v-if="$isMobile" :is="current" keep-alive></prince>

            <div class="pres-web-left" v-if="!$isMobile">
                <div class="pres-web-title">狀態</div>
                <div class="pres-web-list">
                    <div class="pres-web-item" :class="current == 'all'?'start':''" @click="tabClick('all')" >
                        <i class="ation-icon" :style="current == 'all'?{backgroundImage:'url('+require('../../img/dui.png')+')'}:''"></i>
                        <span>{{ $t('message.pre.all') }}</span>
                    </div>
                    <div class="pres-web-item" :class="current == 'payment'?'start':''" @click="tabClick('payment')" >
                        <i class="ation-icon" :style="current == 'payment'?{backgroundImage:'url('+require('../../img/dui.png')+')'}:''"></i>
                        <span>{{ $t('message.pre.payment') }}</span>
                    </div>
                    <div class="pres-web-item" :class="current == 'ongoing'?'start':''" @click="tabClick('ongoing')" >
                        <i class="ation-icon" :style="current == 'ongoing'?{backgroundImage:'url('+require('../../img/dui.png')+')'}:''"></i>
                        <span>{{ $t('message.pre.ongoing') }}</span>
                    </div>
                    <div class="pres-web-item" :class="current == 'evaluate'?'start':''" @click="tabClick('evaluate')" >
                        <i class="ation-icon" :style="current == 'evaluate'?{backgroundImage:'url('+require('../../img/dui.png')+')'}:''"></i>
                        <span>{{ $t('message.pre.evaluate') }}</span>
                    </div>
                </div>
            </div>

            <div class="pres-web-right" v-if="!$isMobile">
                <div class="pres-right-title">我的預約服務</div>
